...
describe("Test Bank Contract", function () {
  ...
  describe("Deposit", function () {
    it("Require if enough funds provided", async function () {
      await expect(
        deployedContract.deposit({ value: hre.ethers.parseEther("0.05") })
      ).to.be.revertedWith("Not enough funds provided");
    });
    it("Should emit Deposit event", async function () {
      await expect(
        deployedContract.deposit({ value: hre.ethers.parseEther("0.2") })
      )
  ...
  describe("Withdraw", function () {
    it("Require if you can withdraw this amount", async function () {
      ...
      await expect(
        deployedContract.withdraw(hre.ethers.parseEther("1"))
